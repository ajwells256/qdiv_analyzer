...
class QualifiedDividendAnalyzer():
    ...
    def get_disqualified_dividends(self, contents: PDFContents) -> "Tuple[Union[list[DividendsInterface], None], Union[DividendsTotal, None]]":
        ...
        if len(cusips) > 0:
            ...
            if len(symbols_with_short_holding_periods) + len(cusips_with_short_holding_periods) == 0:
                print(">>> Congratulations, it seems you had no security holding periods shorter than 60 days. All qualified dividends should remain so.")
                return None, None
            ...
            if (cusip in cusip_to_symbol):
                ...
                for dividend in cusip_dividends:
                    ...
                    if disqualified_sale_count != 0:
                        ...
                        disqualified_shares_count = sum(sale.quantity for sale in disqualified_sales)
    ...
    def get_securities_with_short_holding_periods(sales: "list[SalesInterface]", relevant_cusips: "set[str]") -> "list[SalesInterface]":
        ...
        for sale in sales:
            ...
            if sale.cusip not in relevant_cusips:
                # only perform holding period analysis for qualified dividends
                continue

            if sale.acquired_date is None:
                sale.acquired_date = datetime.strptime(sale.get("acquired_date"), "%m/%d/%y")
            if sale.disposal_date is None:
                sale.disposal_date = datetime.strptime(sale.get("sold_date"), "%m/%d/%y")
            if sale.quanitity is None:
                sale.quantity = atof(sale.get("quantity"))
            ...
            if (sale.disposal_date - sale.acquired_date).days < 61:
